...
public class UserServiceImpl implements UserService {
	...
	public ResponseEntity<UserResult> userSignup(UserDto user) {
		...
		User u = new User(user.getId(),user.getFirstName(), user.getLastName(), user.getEmail(), user.getRole(), encodedPassword,
				user.getDob());
	...
	public ResponseEntity<String> setNewPass(ChangePassDto passChange) {
		...
		if(passwordEncoder.matches(passChange.getOldPass(), user.getPassword()))
		{
			...
			{
				return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("New Password can not be same as old password.");
			}
